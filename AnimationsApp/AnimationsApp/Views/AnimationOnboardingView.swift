...
struct AnimationOnboardingView: View {
        ...
        VStack(spacing: 0) {
            ...
            HStack(spacing: 24) {
                ...
                Button(currentPage == pages.count - 1 ? "Listo" : "Siguiente") {
                    ...
                    else { navigate(to: currentPage + 1) }
                }
            }
    ...
    private func navigate(to page: Int) {
        ...
        withAnimation(.spring(response: 0.5, dampingFraction: 0.78)) { currentPage = page }
    }
}
    ...
    var body: some View {
        VStack(spacing: 32) {
            Image(systemName: page.icon)
                ...
                .symbolEffect(.bounce)
            VStack(spacing: 12) {
                Text(page.title).font(.largeTitle).fontWeight(.bold)
                Text(page.body).font(.body).foregroundStyle(.secondary)
